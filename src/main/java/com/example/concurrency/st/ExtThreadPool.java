...
/**
 * 扩展线程池，关键方法beforeExecute(),afterExecute(),terminated()
 * @author OKali
 * 监控每个执行的开始和结束时间，或者定义其他一些自定义的增强功能
 */
	...
	public static void main(String[] args) throws InterruptedException {
		ExecutorService es = new ThreadPoolExecutor(5, 5, 0, TimeUnit.MILLISECONDS,
			...
			protected void beforeExecute(Thread t, Runnable r) {
				System.out.println("任务准备执行：" + ((MyTask)r).name);
			}
			
			@Override
			protected void afterExecute(Runnable r, Throwable t) {
				System.out.println("任务执行完成：" + ((MyTask)r).name);
			}
